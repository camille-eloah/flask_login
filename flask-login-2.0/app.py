...
def dash():
    if request.method == 'POST':
        ...
        flash('Exercício cadastrado com sucesso!', 'sucess')  

        return redirect(url_for('dash'))

    # buscar todos os exercícios cadastradods
    SELECT = 'SELECT exercicios.nome, exercicios.descricao, usuarios.matricula FROM exercicios JOIN usuarios ON exercicios.usuario = usuarios.id'
    conexao = obter_conexao()
    exercicios = conexao.execute(SELECT).fetchall()
    conexao.close()

    return render_template('dash.html', exercicios=exercicios)
